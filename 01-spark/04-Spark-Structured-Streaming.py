...
# MAGIC %md
# MAGIC ##### 1. Define Streaming Source (Auto Loader)
...
# .option("maxFilesPerTrigger", 1)\ is outdated and Spark 3.x (DBR >= 7.x) will ignore this option completely. 

# COMMAND ----------

# MAGIC %md
# MAGIC ##### 2. Define Transformations on the Stream
...
# MAGIC 
# MAGIC SELECT count(*)
# MAGIC FROM stream_table

# COMMAND ----------

spark.conf.set("spark.sql.shuffle.partitions", "1")  # keep the size of shuffles small

stream_count.writeStream.format("memory") \
  .queryName("stream_count") \
  .outputMode("complete")\
  .trigger(processingTime='10 seconds') \
  .start()
...
# MAGIC %sql select * from stream_count 
...
streamingInputDF = (
  spark
    ...
    .option("loudFiles.includeExistingFiles", "false")
    .json(inputPath)
)
